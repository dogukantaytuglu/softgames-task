...
**Architecture: deliberately simpler than first proposed.** The first design (static `SoundService`, a `SoundConfig` ScriptableObject wrapper, a generic `PlayAudio` trigger component, `Logic`/`Monobehaviour` asmdef split, EditMode tests) was planned in full, then explicitly rejected as over-scoped for a 4-sound take-home: *"maybe too complex what is the simplest audio system we can have I will trigger it from any part of code."* Rebuilt flat (`Assets/App/Sound/`, single `Sound.asmdef`, no Logic split — there's no meaningfully engine-agnostic piece worth isolating here, same reasoning as the `MainMenu` flat-asmdef exception) around two files:
- **`SoundConfig`** (ScriptableObject) mirrors `AudioSource`'s own settings: `clip`, and `volume`/`pitch`/`priority` as **min-max ranges** (a small `[MinMaxRange(min,max)]` attribute + `PropertyDrawer` wrapping Unity's own `EditorGUI.MinMaxSlider` — Unity has no built-in `[MinMaxSlider]` attribute, but does have the underlying widget) that resolve to a fresh `Random.Range` on **every read**, so repeated plays of the same config don't sound identical. Degenerate (`min == max`) ranges reproduce a fixed value.
- **`SoundService`** (static, self-initializing): lazily builds its own `DontDestroyOnLoad` host `GameObject` with an 8-`AudioSource` pool on first `Play()` call — **zero scene wiring required anywhere**, callable from any script immediately. `Play(config)` picks the first non-playing source (steals index 0 if all busy — a pool was chosen specifically over a single shared source after flagging that a literal single source would make the fire's loop and one-shot SFX steal each other). `Stop(config)` tags each source with the config it's currently playing so a looping sound started earlier can be stopped by reference. `IsEnabled`/`SetEnabled(bool)` is a global mute — `PlayerPrefs`-persisted, applied through `AudioListener.volume` (a single engine-wide multiplier) rather than tracking/resuming individual sources, so it instantly silences everything already playing (including an active loop) with no bookkeeping.

**A real, systemic click-sound bug found and fixed across 4 files — not MainMenu-specific despite how it was reported.** `ButtonClickSound` (added to `Assets/App/UI/Prefabs/BaseButton.prefab` directly, so every button variant in the game inherits it — this is exactly the "future click-sound component lands in one place" D44 was prep for) self-wires **additively** in its own `Awake()` (`button.onClick.AddListener(Play)`, never `RemoveAllListeners()` — deliberately, so it can coexist with whatever else is wired to the same button regardless of script execution order). But `HomeButtonController.Awake()`, `MenuButtonSceneLoader.Initialize()`, `PhoenixFlameColorButton.Initialize()`, and `DialogueFinishedView.Wire()` all predated `ButtonClickSound` and used the older `button.onClick.RemoveAllListeners(); button.onClick.AddListener(...)` idiom. Unity does not guarantee `Awake()` order across different components/GameObjects — whichever of these ran *after* `ButtonClickSound.Awake()` on the same button silently wiped its listener. Reported symptom: *"I also dont hear button sounds on the main menu scene load buttons"* — but the same race affects Home, Phoenix Flame's color swatches, and Magic Words' replay/retry buttons equally, just non-deterministically per session. Fixed the first three by dropping the now-redundant `RemoveAllListeners()` (each is only ever called once, so it was never actually protecting against anything). `DialogueFinishedView.Wire` is different — `Show`/`ShowFailure` can legitimately re-run (a retried failed fetch re-wires `retryButton`), so blindly dropping `RemoveAllListeners()` there would risk stacking duplicate retry callbacks instead; fixed surgically by tracking the exact `UnityAction` delegate `Wire` last added and `RemoveListener`-ing only that one.

**A real DOTween bug found in the new `MainMenuAnimationHandler`'s `Join` sequence mode — confirmed empirically, not guessed.** MainMenuScene's intro (3 text reveals, 3 button pop-ins, 7 icon pop-ins — all independently-delayed DOTween Pro `DOTweenAnimation` Inspector components, each self-triggering in its own `Awake()`, "not easy to modify" per the developer) was consolidated into one `MainMenuAnimationHandler` (`Assets/Feature/MainMenu/Scripts/`): a serialized `List<MainMenuAnimationStep>` (`type`: ScalePopIn/TextReveal, `sequenceMode`: Insert/Append/Join, target, `delay`, `duration`, `ease`) builds one real `DOTween.Sequence()` in `PlayIntro()`, driven from `MainMenuInitializer.Awake()` per the project's single-init-point convention. All 13 original steps were ported 1:1 (same targets/delays/durations/eases, mode defaulting to `Insert` — zero visual regression). `Append`/`Join` modes were added on request so new steps can be authored the natural DOTween way instead of hand-computed absolute positions; `Append` used `AppendInterval(delay)` + `Append(tween)` for the pre-tween gap, and `Join` copied the same pattern — which turned out to be wrong specifically for `Join`. Measured directly (reflection on `sequencedPosition`/`Goto()` sampling, via `unity-animation-expert`): `AppendInterval(x)` moves DOTween's internal join-anchor to the sequence's *current end*, and `Join` always attaches there regardless of `x` — a 0.5s delay and a 2.0s delay produced the *identical* start time, and that time wasn't even "joined with the previous step" (delay 0 behavior), it silently degenerated into a zero-gap `Append`. `Append` itself was verified correct (bit-identical to the equivalent `Insert`) and left untouched. Fixed by having `PlayIntro()` track the join-anchor itself (`lastStepStart`, since DOTween doesn't expose its internal one) and `Insert`-ing the joined tween at the computed absolute position instead of relying on `AppendInterval`+`Join`.

**Other real pieces landed the same session:**
- Card-flip sound (Ace of Shadows, `CardView.MoveTo`), fire-loop + color-change sounds (Phoenix Flame — loop started once in `FlameParticle.Initialize`, **stopped in `OnDestroy`**, since `SoundService`'s pool lives on a `DontDestroyOnLoad` object and would otherwise keep looping after leaving the scene entirely), a talking sound on each Magic Words line reveal (`SoundService.Stop` used for the first time here, to cut it off on `OnRevealComplete`).
- Phoenix Flame's 3 color buttons now disable themselves and show a grey overlay for the ~0.85s Animator crossfade (`FlameParticle.IsTransitioning => animator.IsInTransition(0)`, polled each frame by `PhoenixFlameController.Update` — deliberately *not* a duplicated transition-duration field in C#, so it can't drift from the Animator Controller's own value the way the removed `colorTransitionDuration` field once did nothing at all; see the dead-code cleanup below).
- `PhoenixFlameColors.controller`'s transition duration retuned 1.5s → 0.85s; `PhoenixFlameConfig`'s unused `colorTransitionDuration` field (never actually read at runtime, only editable) was removed, along with the custom Inspector's now-dangling `SerializedProperty` reference to it (would have thrown on selecting the config asset otherwise).
- A full-screen invisible `SkipIntroButton` (another `BaseButton` variant) skips `MainMenuAnimationHandler`'s intro via `_sequence.Complete()` — sound explicitly disabled via `ButtonClickSound.isActive = false`, the opt-out field added earlier this same session specifically for this kind of case.
- A `unity-ui-ux-expert` pass caught and fixed the actual defect behind *"[the sound toggle button] didn't match the project"*: `SoundToggleButton.prefab` never overrode `BaseButton`'s sprite, so it rendered as a hard-cornered pure-white square — the only square, only pure-white surface on a screen of rounded warm shapes — and the raw downloaded icon PNGs were pure-black-RGB, structurally untintable to the game's ink color regardless of any sizing fix. Fixed with the missing background/shadow chrome and procedurally-regenerated on-style glyphs (matching how other project glyphs were made, per `Unity_AssetGeneration_GetModels` still returning empty on this install). One judgment call left for review: a hue-matched tan shadow instead of `HomeButton`'s plain black, reasoned from real render comparisons (weak cream-vs-background contrast means the shadow carries the silhouette) but means the two icon buttons' shadows now differ slightly.

**Real Unity/tooling gotchas hit this session, worth remembering:**
- **Unity's audio importer doesn't support `.m4a`** (only `.wav`/`.aiff`/`.mp3`/`.ogg` + tracker formats) — a dropped/copied `.m4a` imports silently as `DefaultImporter`, not `AudioImporter`, with no error. `ffmpeg` (installed via `winget install Gyan.FFmpeg` this session) converts cleanly to `.wav`.
- **Windows drag-and-drop into the Unity Editor can silently no-op** if the Editor and the source window run at different privilege levels (e.g. Editor elevated, Explorer not) — copying the file via the terminal sidesteps it entirely.
- A large, rarely-changing looping clip (`FireLoop.wav`, ~30s) defaults to `DecompressOnLoad`/quality-1 on import — fine for a short one-shot, wasteful (full PCM in RAM, larger build) for something this long; switched to `CompressedInMemory`/Vorbis 0.6.

**Left open, flagged not fixed:** `Assets/App/Sound/Sprites/volume.png` and `sound-off.png` (the original rejected icons) are now fully unreferenced dead weight; the new sound-toggle glyphs aren't in `UI.spriteatlas` (2 extra draw calls, deliberately kept out rather than expose them to the `enableRotation: 1` 9-slice bug D40 already flagged on that atlas); the sound-toggle button isn't part of `MainMenuAnimationHandler`'s intro pop-in list, so it's the one element on that screen that doesn't animate in.

---

### D45 — Magic Words: progress-pill fill fixed and tweened; two real dialogue bugs root-caused via live pair-debugging, one fixed (2026-08-28)

Started from a UI bug report: the `LINE n OF m` progress pill's fill didn't fully fit inside its track. Turned into a full debugging session on the dialogue screen itself, working live with the developer stepping through in Rider.

**Progress pill: fixed, committed, pushed (`8cf2b99`).** `Fill` and `Track` both used a 9-sliced capsule sprite, but `Fill` was `Image.Type.Filled` — which ignores the sprite border entirely and stretches the rounded caps into a taper at low fill amounts (confirmed by rendering the canvas before/after; at `fillAmount = 1/17` the fill degenerated into a thin pointed needle poking past the track's own rounded cap). Fixed by switching `Fill` to `Sliced`, overlapping `Track` exactly, and driving width via `RectTransform.anchorMax.x` instead of `fillAmount` — `DialogueProgressView.SetProgress` now sets `fillRect.anchorMax = new Vector2(progress, ...)`. Then tweened (0.3s `Ease.OutBack`, clamped so the overshoot can't push the fill past the track's end on the last line, `DOKill`-guarded against overlapping taps) instead of snapping.

**Two real, separate dialogue bugs, found by debugging live rather than guessing.** The reported symptom was "first two dialogue texts don't become visible, and it gets stuck on the second one." Debug logging (added, then removed once no longer needed — twice, since the second bug needed more) traced this to two independent root causes:

1. **Invisible text: `LeftDialogueBox`/`RightDialogueBox` are children of `LeftPortrait`/`RightPortrait`**, and `SpeakerPortraitView.Hide()` — called from `Initialize()` at `Awake()` — deactivates the portrait GameObject at startup. The only thing that reactivates it, `Bind()`, was only reachable from the async avatar-load coroutine callback, which resolves *after* `ShowLine` has already called `box.PlayReveal()` synchronously in the same frame. `PlayReveal`'s `ForceMeshUpdate()` can't rebuild character/mesh data for a TMP component that isn't active *in the hierarchy* — it silently no-ops, `textInfo.characterCount` reads `0`, `duration` computes to `0`, and the reveal "completes" instantly with nothing ever shown. **Root-caused to a specific commit via git archaeology, not guesswork:** `4388204` (2026-08-27, "add portrait groundwork") introduced the whole system in one internally-consistent shot — box-under-portrait parenting *and* a synchronous `portrait.Bind(name, null)` call in `ShowLine`, which as a side effect kept the portrait active in time for every line's reveal. `4bc966e` (2026-08-28, the developer's own commit, *"stop forcing the portrait into its missing-avatar look before the fetch resolves"*) removed that synchronous call for a legitimate, unrelated reason (it was flashing the "missing avatar" placeholder for slow-loading avatars) — but it was also the only thing keeping the portrait active in time, and nobody knew that until this session. Classic load-bearing side effect: right fix for the stated problem, silent regression on an unstated dependency.
2. **Auto-advance getting stuck: a reentrancy bug in the vendored `TimerUtil` plugin**, not Magic-Words-specific. `CountdownTimer.OnTimeout()` (`Assets/Plugins/TimerUtil/Scripts/CountdownTimer.cs`) called `OnCountdownComplete?.Invoke()` then `Stop()`, in that order. When the invoked callback synchronously restarted the *same* timer (which is exactly what `NextDialogue → ShowLine → PlayReveal → OnRevealComplete → _autoAdvanceTimer.Start()` does whenever `PlayReveal` completes synchronously — i.e. bug 1's exact trigger condition), the outer `OnTimeout()` call's own trailing `Stop()` ran *after* that reentrant restart, on the *same* timer instance, and immediately killed the timer that had just been freshly started for the next line — before `TimerTicker` ever got a chance to tick it. No exception, nothing in the console; from the outside it just silently stopped advancing. Confirmed live: a manual tap "fixed" it exactly once per occurrence (a `Button.onClick` handler is a clean, non-nested call stack, so its `Start()` isn't reentrant and doesn't get clobbered) — consistent with the theory before the fix was even written. **Fixed** by reordering to `Stop()` then `Invoke()`, and only calling `Destroy()` if `State` is still `Stopped` after the callback runs (so a reentrant restart isn't destroyed either). This protects every `CountdownTimer` consumer in the project, not just this one call site.

**The developer's own fix for bug 1 sidesteps it rather than patching it.** `DialogueBoxView.PlayReveal` was rewritten around DOTween Pro's `DOText` shortcut, replacing the old `ForceMeshUpdate()` + `textInfo.characterCount` + `DOMaxVisibleCharacters` approach entirely. `DOText`'s rich-text-tag-aware reveal parses the plain string itself (so a `<sprite name="...">` emoji token still reveals as one atomic step) — it has no dependency on the live TMP mesh or the GameObject's active-in-hierarchy state at all, so the exact failure mode in bug 1 (an inactive hierarchy making `ForceMeshUpdate()` a no-op) can't occur through this code path anymore. The box-under-portrait parenting and the `Hide()`/`Bind()` timing dependency are still there structurally — just no longer able to manifest as invisible text specifically.

Along the way, the `PlayReveal(string, float charactersPerSecond, Action)` parameter was found to be mis-passed straight into `DOText`'s `duration` parameter (a whole-tween duration, not a per-character rate) — the field was already behaving as a flat per-line duration despite its name, so it was renamed to `duration` end-to-end (`MagicWordsConfig`, `PlayReveal`, the call site) to make the name honest. Real, not just cosmetic: this trades away the original per-character-count pacing (a short line and a long line now take the same fixed time to reveal) for a simpler flat duration — not reversed as of this entry.

**Two smaller bugs surfaced on review, flagged but not fixed:**
- The `charactersPerSecond` → `duration` rename has no `[FormerlySerializedAs]`, so `MagicWordsConfig.asset`'s old `charactersPerSecond: 2.2` YAML key doesn't map to the new field. Verified live: `MagicWordsConfig.Duration` currently reads `2` (the C# default), not the tuned `2.2` sitting orphaned in the asset. Needs the asset resaved through the Inspector, or the attribute added.
- `CompleteRevealImmediately()` is now `DOTween.Kill(dialogueText, true)`, and `complete: true` fires the tween's own `OnComplete` — which is the same `OnRevealComplete` that `MagicWordsController.OnAdvanceClicked` then calls again explicitly right after. Double-fires every time a player taps mid-reveal. Currently harmless purely because `Timer.Start()` no-ops when already started, not because it's guarded by design.

**Not yet confirmed end-to-end.** The two root causes are diagnosed and one is fixed (`TimerUtil`); the developer's `DOText` rewrite should sidestep the other, but a full clean run through all 17 lines with both fixes together, by the developer's own eyes, hasn't happened as of this entry.
